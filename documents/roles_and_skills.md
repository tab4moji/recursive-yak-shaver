...
2. **自動組み上げ**: `Phase 5` (Generator) は、選択されたパターンの入出力モードに基づき、適切な Bash フレームワーク・コードを自動的に適用してスクリプトを構築する。

これにより、LLM は複雑なシェル操作をゼロから考える必要がなくなり、実績のある安定したパターンを選択・適用するだけで確実にタスクを遂行できるようになる。
