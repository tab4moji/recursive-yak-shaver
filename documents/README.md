...
- **チートシート・スキーマの刷新**: `input` フィールドを配列化し、名前・説明・型を明文化。
- **ロール固有のコンテキスト注入**: `rys/role_utils.py` により、フェーズごとに必要な情報だけを抽出して LLM に提示（Dispatcher は高レベル計画、Coder は実装詳細）。
- **TOON 形式の最適化**: 複雑な引数構造を平坦な文字列へ変換し、モデルの理解度を向上。
